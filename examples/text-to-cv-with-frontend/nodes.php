...
class CreatePlanNode extends Node {
    ...
    public function exec(mixed $prompt): string {
        $systemPrompt = "You are a helpful assistant. Based on the user's request, create a structured plan for a CV. The plan should be in YAML format. For example:\nsections:\n  - personal_details:\n      name: John Doe\n      email: john.doe@example.com\n  - summary: A brief professional summary.\n  - experience:\n      - position: Senior Developer\n        company: Tech Inc.\n        years: 2020-2024\n        description: Description of responsibilities.\n  - education:\n      - degree: BSc Computer Science\n        university: University of Example\n        years: 2016-2020\n";
        return callLlm($prompt, [['role' => 'system', 'content' => $systemPrompt]]);
    }
...
class EditPlanNode extends Node {
    ...
    public function exec(mixed $p): ?string {
        ...
        $systemPrompt = "You are a helpful assistant. The user wants to edit a CV plan. Based on the user's edit prompt, update the following YAML plan. Output only the updated YAML plan.";

        return callLlm($p['edit_prompt'], [
...
class ConvertToPdfNode extends Node {
    ...
    public function exec(mixed $p): string {
        ...
        return convertHtmlToPdf($p, $filename);
    }
